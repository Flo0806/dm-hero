...
import { localeEnglishName, type AppLocale } from '~~/types/locale'
...
export default defineEventHandler(async (event): Promise<GenerateNameResponse> => {
  ...
  // de/en/zh-CN have native prompts; every other language uses the English prompt with an explicit reply language
  const replyIn = language === 'en' ? '' : ` Reply in ${localeEnglishName(language)}.`
  const systemPrompt
    = language === 'de'
      ...
      : language === 'zh-CN'
        ...
        : `You are a creative D&D name generator. Generate fitting, atmospheric names for fantasy characters and locations.${replyIn}`
  ...
  const userPrompt = language === 'de'
    ...
    : language === 'zh-CN'
      ...
      : `Generate a fitting ${label?.en}${body.context ? ` for: ${body.context}` : ''}. Reply ONLY with the name, no explanation.${replyIn}`
