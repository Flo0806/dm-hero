...
### Nine Themes to Suit Your Table

DM Hero ships with **9 color themes** — pick the mood that fits your world:
...
- **Music "Sunset Beat"** :badge[v1.5]{type="info"}: retro sunset orange and magenta over deep stage blue — with music notes floating across the dashboard
...
Every minor release now carries a codename, shown right in the title bar. This one: **Bardic Inspiration**.
...
Optionally connect an AI assistant (Claude, Cursor, …) and let it create content **directly in DM Hero**. **1.5** extends what it can help with considerably — it stays optional, DM Hero works fully without it.
...
### What It Can Do :badge[v1.5]{type="info"}

Ask it *"What can you do?"* and it tells you. In short:

- **Entities** with metadata, tags, folders, relations — and **cross-links inside descriptions** that resolve to real `{{npc:123}}` links on import
- **Portraits & cover images** from a file on your computer or a URL; **documents** (markdown or PDF) attached to entities
- **Sessions** — create them, write summaries and notes, set the in-game date
- **Groups**, **maps** (upload the region map, place NPCs as markers and locations as areas) and **encounters** with participants and HP
- **Read back** what exists, **archive** or **delete** (always with a preview first)

A realistic request: *"Here's the one-shot PDF — create the campaign, all NPCs and places with portraits from the PDF's folder, the region map with markers, the ambush encounter and session 1 with the intro text."*

### Safe by Design

- The AI first shows a **dry-run preview** of exactly what it would create or change — nothing is written until you confirm.
- It can **link to entities that already exist** (no duplicates — the preview warns about same-name entities) and even **edit** existing ones when you ask.
...
### Your Soundtrack, at the Table

The headline of **1.5 "Bardic Inspiration"**: DM Hero plays your own music library. Tavern ambience, battle drums, eerie dungeon drones — straight from a folder on your disk, no upload, no cloud.

### Setting Up

1. Open **Music** in the sidebar and click **Choose folder**.
2. **Desktop app**: a native folder dialog opens. **Docker**: mount your music as a volume and enter the path inside the container (e.g. `/music`).
3. DM Hero scans the folder. Every sub-folder becomes a collapsible playlist; supported formats are mp3, m4a, aac, flac, wav, ogg, opus, webm and aiff.

### Scenes

Hover a folder and click the **star** — it becomes a *scene* and shows up as a chip at the top of the music page: *Tavern*, *Forest*, *Boss Fight*. One click starts it. Switching scenes mid-song **crossfades** instead of cutting.

### Playback

- **Repeat folder** (default): stays inside the current folder and loops it — never drifts into the next folder and spoils the mood. Also **Repeat track** and **Play everything**.
- **Shuffle**, **Crossfade** (1–12 s) and a **volume** slider.
- DM Hero remembers the last track and position — after a restart, *Play* resumes where you left off.
- Missing or deleted files are skipped and struck through; a missing folder shows a hint with a **Rescan** button.

### The Mini Player

Leave the music page and a **floating mini player** follows you: draggable along the bottom edge (it never covers the sidebar or a "+" button), with play/pause, skip, ±10 s, volume, a **queue popup** (click the title) showing what's next, a **compact** mode and a **minimise** button that shrinks it to a single note bottom-left. The sidebar's *Music* entry also gets ◀ ⏯ ▶ while something is playing.

### Keyboard Shortcuts

They work on every page while a track is loaded (never while typing):
...
Prefer YouTube, Spotify or Tabletop Audio? Attach **playlist links** to a session (*Session → Music* tab). They show as chips on the session card and open in your **system browser**, so the music keeps running while you work in DM Hero. In the desktop app, all external links now open in the browser instead of the app window.

---

## Six Languages :badge[v1.5]{type="info"}

DM Hero speaks **German, English, Italian, French, Spanish and Simplified Chinese** (a community contribution — thank you!). Switch anytime from the flag menu in the top bar. Spotted a wrong term? Translations live in `packages/app/i18n/locales/` — pull requests are very welcome.
